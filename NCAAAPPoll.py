...
try:
    rankhistory = pd.read_csv('/Users/Ryan/Google Drive/ncaa-basketball-data/appollhistory.csv')
    firstpoll = rankhistory['PollNum'].max() + 1
    importedlen = len(rankhistory)
    print('Imported appollhistory.csv, beginning scrape at poll #' + str(firstpoll))
except:
    rankhistory = pd.DataFrame()
    firstpoll = 539 #539
    print('No appollhistory.csv found, beginning scrape at poll #' + str(firstpoll))
...
# Split poll name
rankhistory['PollDate'] = rankhistory['PollName'].str.extract(
                                                r'(.*).AP Men.*'
                                                , expand = True)
# TODO get dates of preseason and final polls into PollDate

# Extract season from preseason/final
rankhistory.loc[rankhistory['PollDate'].str.contains('Preseason') |
                rankhistory['PollDate'].str.contains('Final')
                ,'Season'] = rankhistory['PollDate'].str[0:4]

# Extract season from in-season polls
rankhistory['temp'] = pd.to_datetime(rankhistory['PollDate'],errors = 'coerce')+ timedelta(days=180)
rankhistory.loc[rankhistory['temp'].notnull(),'Season'] = rankhistory['temp'].astype(str).str[0:4]
