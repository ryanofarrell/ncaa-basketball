...
###############################################################################
rsgc = pd.read_csv(
    filepath_or_buffer='/Users/Ryan/Google Drive/ncaa-basketball-data/2018-kaggle-update/RegularSeasonCompactResults.csv'
)
rsgd = pd.read_csv(
...
TourneySeeds = pd.read_csv(
    filepath_or_buffer='/Users/Ryan/Google Drive/ncaa-basketball-data/2018-kaggle/NCAATourneySeeds.csv'
)
TourneyResults = pd.read_csv(
    filepath_or_buffer='/Users/Ryan/Google Drive/HistoricalNCAAData/TourneyCompactResults.csv')
...
rsg = pd.merge(
    ...
    how='left',
    on=[
        'GameDate', 'Season', 'WTeamID', 'LTeamID', 'WScore', 'LScore', 'WLoc',
        'NumOT'
    ])
del rsgc, rsgd
...
rsg_summary1 = rsg[['Season', 'GameDate']].groupby(
    ['Season']).agg('count').reset_index()
rsg_summary2 = rsg[['Season', 'DetailedGame']].groupby(
...
rsg['TmPoss'] = rsg['TmFGA'] \
                - rsg['TmOR'] \
                + rsg['TmTO'] \
                + .475 * rsg['TmFTA']
rsg['OppPoss'] = rsg['OppFGA'] \
                - rsg['OppOR'] \
                + rsg['OppTO'] \
                + .475 * rsg['OppFTA']
...
for x in {'Opp', 'Tm'}:
    for column in metrics:
        rsg[x + column + 'per40'] = rsg[x + column] / rsg[x + 'Mins'] * 40
...
def opponentadjust(prefix, coremetric):
    global rsg_currseason, st_currseason
    ...
    if st_currseason[otherprefix + coremetric + 'perGame'].isnull().values.any():
        st_currseason['OA_'+ prefix + coremetric + 'perGame'] = np.NaN
        st_currseason['OA_'+ prefix + coremetric + 'per40'] = np.NaN
        st_currseason['OA_'+ prefix + coremetric + 'perPoss'] = np.NaN
    else:
        ...
        temp_st_currseason = st_currseason[[
        ...
        temp_st_currseason = temp_st_currseason.rename(
        ...
        rsg_currseason = pd.merge(rsg_currseason, temp_st_currseason, 
                                  left_on=['OppName', 'Season'], 
                                  right_on=['TmName', 'Season'], 
                                  how='left', 
                                  suffixes=('', '_y'))
        del rsg_currseason['TmName_y']
        ...
        rsg_currseason['GameOppAdj_' + prefix + coremetric ] = \
            rsg_currseason[prefix + coremetric ] - \
            rsg_currseason['OppAvg_' + otherprefix + coremetric + 'perGame']
            
        del rsg_currseason['OppAvg_' + otherprefix + coremetric + 'perGame' ]
        # Inverse it for when you start with an opponent, to make positive numbers good
        if prefix == 'Opp':
            rsg_currseason['GameOppAdj_' + prefix + coremetric ] = \
                rsg_currseason['GameOppAdj_' + prefix + coremetric ] * -1
        ...
        temp_st_currseason = rsg_currseason.groupby(['TmName', 'Season'])[
            'GameOppAdj_' + prefix + coremetric  ].sum().reset_index()
    
        # bring that value back into iteams, adjust for a 40-min game
        st_currseason = pd.merge(st_currseason, temp_st_currseason, 
                                  on=['TmName', 'Season'], 
                                  how='left')
        st_currseason = st_currseason.rename(
            columns={'GameOppAdj_' + prefix + coremetric : 'OA_' + prefix + coremetric })
        
        # Get perGame, perPoss and per40 multipliers
        st_currseason['OA_'+ prefix + coremetric + 'perGame'] = \
            st_currseason['OA_'+ prefix + coremetric ] / \
            st_currseason[prefix + 'Game']
        st_currseason['OA_'+ prefix + coremetric + 'per40'] = \
            st_currseason['OA_'+ prefix + coremetric ] / \
            st_currseason[prefix + 'Mins'] * 40
        st_currseason['OA_'+ prefix + coremetric + 'perPoss'] = \
            st_currseason['OA_'+ prefix + coremetric ] / \
            st_currseason[prefix + 'Poss']
    
    print(prefix + coremetric)
#    del iteams['TmName_y']
###############################################################################
###############################################################################
...
# Initialize seasonteams dataframe
seasonteams = pd.DataFrame()

# Loop through each season in rsg_summary
for currseason in rsg_summary['Season']:

    # Limit the rsg data to just the current season
    rsg_currseason = rsg.loc[rsg['Season'] == currseason]

    # Create current season seasonteams, summing all summables
    # TODO why is summables not summing GameMins?????????????
    st_currseason = rsg_currseason.groupby(
            ['TmID', 'TmName'])[summables].sum().reset_index()
    st_currseason_wtf = rsg_currseason.groupby(
            ['TmID', 'TmName'])['TmMins','OppMins'].sum().reset_index()
    st_currseason = st_currseason.merge(
            st_currseason_wtf,
            how='inner',
            on=['TmID','TmName'])
    del st_currseason_wtf

    # Add currseason column to seasonteams_currseason
    st_currseason['Season'] = currseason
    
    # Get per-game season stats into st_currseason (can't just sum per-games since it will incorrectly weight some games)
    # 22 metrics, should add 44 cols to st_currseason
    for x in {'Opp','Tm'}:
         for column in metrics:
             st_currseason[x + column + 'perGame'] = st_currseason[x + column] / st_currseason[x + 'Game']
             st_currseason[x + column + 'per40'] = st_currseason[x + column] / st_currseason[x + 'Mins'] * 40
             st_currseason[x + column + 'perPoss'] = st_currseason[x + column] / st_currseason[x+'Poss']
    del column, x
    ...
    seasonteams = seasonteams.append(st_currseason)
...
# TODO everything below this is old
# =============================================================================
# # Add MArch 11 games
